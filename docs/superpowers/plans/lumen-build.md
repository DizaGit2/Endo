...
- [ ] **T26 — riverpod retry masks every designed read-error state (app-wide). NEW 2026-08-24, from T22a's F-10.** `ProviderContainer.defaultRetry` declines to retry only `ProviderException`/`Error`, so a **`Failure` is retried up to 10 times over ~38 s**, and `ProviderElement.triggerRetry` emits `AsyncLoading(error:…, retrying: true)`, which `AsyncValue.when` routes to **LOADING**. **Every `AsyncNotifier` screen shows a spinner instead of its designed read-error body** until retries are exhausted — so P4b's *"designed error/retry states"* exit criterion is substantially untrue for the READ half. **Why nobody noticed:** `day_detail_controller_test.dart`'s `isA<AsyncError>` assertions **pass only because those containers have no subscriber**, so the element is disposed and `triggerRetry` short-circuits — the tests are green for a reason that does not exist in production. Pre-existing since the P3a foundations; app-wide blast radius, so it is its own task and not a rider. Commit `fix(client): do not retry a Failure into a spinner`.
