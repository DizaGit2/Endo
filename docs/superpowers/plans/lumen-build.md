...
- [ ] **T20 — Screen 12 symptom form.** Pain row + LOCATION/TYPE/TRIGGERS chips + notes (≤2000) + RELATED chips with per-chip intensity (R-12, R-14); assembles the whole batch including the body-map points returned by T21; per-field errors bound from `ValidationFailure.fields`. **SPLIT INPUT (symptoms-cluster survey): T20 as originally written debuted ELEVEN things in one commit — the first selectable chip, the first repeated-instance `LumenIntensityScale` layout (up to 21 instances each needing its own semantics label), the first batch write, the first index-keyed error binding, a `LumenInputField` change, the first long-form + sticky-CTA pattern, a new mount pattern for a screen with three inbound points across two branches, three R-20 affordances in other features, the R-18 cap, four label-map promotions, and registry registration. It fails both criteria that split T16, by a wider margin. The widget and label debuts are now T19b/T19c; the edit path is cut with `PUT`. What remains: the form, the four chip rows in frozen vocabularies, the pain row, per-chip intensity, notes, batch assembly + POST, per-field error binding, the R-18 and empty-batch guards, the mount decision, and the three R-20 affordances — which must ride here because this is the commit that first makes screen 12 reachable.** **`LumenFieldLabel` takes SENTENCE case and uppercases for display while announcing the string unchanged — pass `Location`, never `LOCATION`, or a screen reader spells it out (the exact T8 defect the widget exists to prevent).**
