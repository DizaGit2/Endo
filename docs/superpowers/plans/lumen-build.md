...
- [ ] **T16 — Screen 11 day detail.** **Hazard inherited from T15, and T16 is the first task that can trigger it:** a `refresh()` arriving while `_page` is mid-flight sees `state.value == null` and takes the `invalidateSelf()` branch, which **re-derives today's month** — the snap-back that `refresh()` exists to avoid. Deliberate and documented at `cycle_calendar_controller.dart:217-219`, but a day write landing during a month page would bounce the user's calendar back to today. `GET/POST /cycle/day/{date}` (MERGE — **no clear affordance**), the day's symptoms via `GET /symptoms?from=D&to=D`, and the period-event editor that owns `POST /cycle/events` (**FULL UPSERT — re-hydrate `notes` and `flowIntensity` or they are wiped**). Mood only; **energy and activity sections are dropped** (D-10 defers energy; activity is P5).
...
**STATUS**
- **State:** IN_PROGRESS · **Branch:** `phase/04b-logging-client` (from `bc73237`) · **Tasks:** 20/32 (T1–T15 + the six review-born inserts T5b/T5c/T5d/T8b/T9b/T14b; five screens shipped — 3, 4, 5, 6, 7 — and onboarding is complete end to end)
