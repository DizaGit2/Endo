...
- [ ] **T25 — Phase close.** Full verify sweep, the manual on-device walkthrough (R-06 (ii)) pasted into STATUS — **the step-by-step script is [`../specs/2026-05-31-build-strategy/p4b-manual-walkthrough.md`](../specs/2026-05-31-build-strategy/p4b-manual-walkthrough.md)** (written at T24; its commands are verified against **Windows PowerShell 5.1**, this project's documented shell, after three of them were found to be bash-only), coverage re-measured (the floor stays **60.0** for the phase — raising a gate mid-phase converts it into a schedule risk; report the observed number and let acceptance ratchet it). **Client coverage is not reproducible run-to-run:** T3 measured the *same commit* twice and got 89.80% and 90.20% (~5 lines of 1245), from timing-dependent branches on the async router/gate paths T1 added. **Do not ratchet the floor to a tight number against that baseline** — either keep a multi-point buffer, or first make those timers deterministic in tests (fake clock) so the number stops moving. A tight floor over a noisy baseline produces flaky CI failures unrelated to any code change, contract byte-identity proven, STATUS filled, ledger → NEEDS_REVIEW. **Use a freshly created account for the on-device walk** — 14 `user_keys` rows in the local dev DB are permanently undecryptable after the T20 incident, and a `@lumen.test` account among them would fail at `GET /me` for reasons that have nothing to do with P4b.
...
**STATUS**
- **State:** IN_PROGRESS · **Branch:** `phase/04b-logging-client` (from `bc73237`) · **Tasks:** 42/43 (T1–T21b + T16b/T16c/T16d/T22c/T17b/T22a/T25a/T22b/T23/T26/T24 + the review-born inserts T5b/T5c/T5d/T8b/T9b/T14b/T19b/T19c; **eight screens shipped — 3, 4, 5, 6, 7, 8, 10, 11, 12** — onboarding complete end to end and the symptoms cluster complete. **Denominator corrected:** the running total was wrong before this session — at T20b the plan said 29/**37** while this section already held **38** checkboxes; the T21 split made **39**. Counted from the file, not carried forward.)
