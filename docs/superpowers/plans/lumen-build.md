...
| B-25 | The `TZ=Europe/Madrid` CI step ran and passed (**77 tests**) — but T17b's own requirement, *"This step MUST be confirmed by a deliberate red before it counts as a control"*, is **recorded nowhere as done**. Until someone puts `.subtract(const Duration(days: 1))` back on a throwaway branch and watches it fail, the step could be a silent no-op — the exact false-green shape this phase kept producing. **R8 forbade me doing it here** (it is a code change) | `ci-client.yml` | M | needs an owner |
