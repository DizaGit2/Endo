...
- [ ] **T21 — Screen 13 body map.** R-15 geometry, front/back, per-point intensity, region chip list as the non-tap path; returns points to screen 12. **Carries one deferred minor from T20a's review:** the assembler attaches the episode's single note to `drafts[0]`, so a body-map point that ever carries its own note would be **silently overwritten** by screen 12's notes box on a body-map-only save. Inert today (the body-map seam is empty until this task fills it) — which is exactly why T21 must decide it deliberately rather than discover it. **Also see the T21 pre-survey in the SDD ledger: the front/back region-membership table is specified in NO source, and inventing one shades toward an anatomical claim; the leading candidate is to treat `side` as the view the user was looking at rather than our claim about where a region lives, since the server imposes no cross-field rule between `region` and `side`.**
...
**STATUS**
- **State:** IN_PROGRESS · **Branch:** `phase/04b-logging-client` (from `bc73237`) · **Tasks:** 25/37 (T1–T18 + T14b + the review-born inserts T5b/T5c/T5d/T8b/T9b/T14b; five screens shipped — 3, 4, 5, 6, 7 — and onboarding is complete end to end)
