...
- [ ] **T22a — Screen 32, settings body.** YOUR PATTERN editable (the only surface that can ever set `avgPeriodLengthDays`); PREDICTIONS toggles; send only changed fields — **the response is deliberately not round-trippable and echoing it back is a 400**; empty body is a 400; the two non-blocking sanity-warning codes render as an **advisory note after a successful save**, never as a form validator that blocks submit (that would reintroduce the entry blocker the PO forbade — R-17); first-day-of-week row dropped (R-10); retrain footer dropped (R-16).
