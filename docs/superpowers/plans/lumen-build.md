...
**NEXT PHASE TO RUN: P4a — Backend Onboarding-rest + Cycle + Symptoms (gated: needs the pre-P4a decision session + the long-lead gates below).**  (P0a, P1, P2, P3a, P3b, **P3c DONE** — P3c accepted 2026-07-08 (human-reviewed; the on-device walking skeleton was watched live on the emulator) and merged to `main` via PR #3, tag `phase-03c`. **Before starting P4a, run the pre-P4a decision session** (D-02, D-08..D-14, B15/B16 + `definitions.md` extracts; ratify D-01, close D-04) and keep working the long-lead gates below — clinical outreach C-01..C-13, the Anthropic DPA, and legal L-01/L-02/L-04 are the schedule's real critical path.)
**Plan revision:** r14   **Repo HEAD when ledger last updated:** `main` @ P3c merge (`phase-03c`; P3c = 19 commits merged --no-ff)
...
- **Clinical asks C-01..C-13** ([clinical-asks](../specs/2026-05-31-build-strategy/clinical-asks.md)): all blank, zero sign-offs; needs a real clinician. Gates **P6** (C-01..C-05, C-09..C-12) and **P7b** (C-06..C-08); C-13 pre-launch. **#1 schedule risk — send the pack now (noted 2026-07-06).**
...
- **Pre-P4a decision session** (fast-path like D-05/06/07 on 2026-06-14): **D-02**, D-08..D-14, B15/B16 + `definitions.md` extracts; ratify D-01 (P3b shipped its default — social buttons omitted); close D-04. See P4a Preconditions.
...
- **Preconditions:** P4a endpoints green + regenerated client; decision D-14 (goal→dashboard mapping may stay ungated v1); P3c a11y `Semantics` pattern + guard tests in place (r13).
- **Exit criteria:** screens 3–14, 32 work against the live backend in both themes; designed error/retry states on write screens 9/11/12/13; **every new screen ships `Semantics` per the P3c pattern (guard green)** (r13); **authed-but-not-onboarded users are routed into the onboarding flow — `MeResponse.onboardingCompleted` consumed, `/onboarding` registered (closes the P3b T5 stub)** (r13); goldens + integration_test green; drift guard clean.
- **Tasks (outline):** onboarding screens 3–7 (resume guard) → dashboard/calendar/day-detail/phase-correction → quick check-in bottom sheet → symptom form + body map (snap-to-region per D-09) → cycle settings → screen-2 client-side validation + `cachedRead` in-flight de-dup (r13 deferrals) → goldens + integration. *(Detailed at phase entry.)*
