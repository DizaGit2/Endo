...
#### T20 rulings (2026-08-11) — READ BEFORE T21 REGENERATES THE CLIENT

**1. Query-parameter `required`: nothing changes, deliberately.** `GET /symptoms` validates `from`/`to` as mandatory but documents neither as `required`, so the T21 client takes all four window parameters as optional named arguments. **Measured at T20:** `[Required]` (DataAnnotations) and `[BindRequired]` on a nullable minimal-API parameter do **not** flip ApiExplorer's `IsRequired` — the emitted parameter was byte-identical with and without them, because `EndpointMetadataApiDescriptionProvider` derives it from the parameter's nullability and default alone. The two remaining levers are both worse than the understatement: de-nullifying the handler parameters *does* flip it but moves the rejection from the validator to the **binder**, costing the 400 its field attribution and reinstating precisely the shape §G12 gave T3 to eliminate; and a hand-maintained Swashbuckle `IOperationFilter` under `backend/src` would restate, in a second place that can drift, a rule the validator already owns. The failure this leaves open is loud, immediate and field-attributed (400 keyed on `from`/`to`; `SymptomListServiceTests` covers it) — the opposite of the silent T13 defect. Pinned by `OpenApiContractTests.OpenApi_leaves_the_symptoms_window_parameters_optional_because_the_handler_binds_them_nullable`, whose real job is to fail if someone later "fixes" the ergonomics by de-nullifying the parameters. `GET /cycle/calendar`'s bounds are genuinely optional (they default to the user's current month), so it needs no ruling.

**2. The systemic sibling — no schema in the document emits a `required` array, so every generated Dart property is nullable.** First seen in T1's spike; still true across all 45 schemas at T20. Also unchanged, and deliberately **not** guarded: a test pinning it would freeze an accident. Flipping it means `SupportNonNullableReferenceTypes()` plus `[Required]`/non-nullable members across ~45 DTOs, which makes the built_value properties non-nullable — so any response omitting a key becomes a client-side **deserialization failure** instead of a null. P4a's DTOs are nullable *by design* (D-02 makes every onboarding answer skippable), and no backend test can observe the built_value consequence without running the real generator. **P4b therefore inherits `T?` on every generated property and must null-check.** If a later phase wants required members, do it deliberately with the generator in the loop, not as a Swashbuckle flag flip.

**3. The nullable+`default` rule is now document-wide.** `OpenApiContractTests.OpenApi_declares_no_schema_that_is_both_nullable_and_defaulted` walks every object in the emitted tree and needs no member list, so it cannot fall behind the document. The three per-schema lists from T13/T14/T15 stay as documentation of why each specific member matters.

**4. `AssertSpineEndpoints` is now three-way and partly route-table-derived** (document operations == route table operations == the declared `Spine` list). Adding an endpoint after T20 fails it until the route is declared in `OpenApiSnapshotTests.Spine` — in the same commit as its `TenantIsolationLiveTests.Swept` entry.
