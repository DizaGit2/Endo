...
| Feature-folder DTO files `<Feature>/<Feature>Contracts.cs` (**no `namespace`** → global namespace preserved) | **T3** sets the convention | all endpoint tasks follow it. "DTOs at the bottom of `Program.cs`" is superseded — P4a adds ~45 records. |
...
| `DeviceRegistrationService` | **T15** | T17's `/onboarding/notifications` reuses it |
| `ConcurrencyRetry` helper | **T10** | T15 reuses it |
...
- **Production changes**
  ...
  - `POST /onboarding/complete` — no body. Mandatory set = a live `users` row **and ≥ 1 live `cycle_events` row with `Kind = period_start`** (checked on the *data*, so a user who logged a period through the cycle module also qualifies); missing → **409** `code=onboarding_incomplete`, `extensions.missingSteps = ["cycle"]`. Otherwise: **guarded claim** `Where(u => u.Id == userId && u.OnboardingCompletedAt == null).ExecuteUpdateAsync(...)` (the `CryptoShredJob` idiom) so concurrent calls stamp exactly once, then **backfill defaults for any table with zero rows** — 5 goals (2 selected), 7 hormone prefs (**all** charted), 4 notification prefs (ON/ON/OFF/OFF), 1 cycle-settings row (28 / null / `somewhat`) — **in the same transaction** as the stamp, so `GET /me` can never report `true` while defaults are missing. `user_insight_snapshot` gets **no row** (§G6). A second call → **200** with the **original** `completedAt` and `alreadyCompleted: true`. Add a `// TODO(C-12)` beside the mandatory check for the amenorrhea branch, explicitly **not** in P4a.
