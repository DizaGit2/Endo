...
- **Entities:** none owned; writes to `users`, `user_profile_enc`, `body_metrics` (the screen-4 `weight_kg` seed — P4a/T16; rider 4 keeps weight's one source of truth here rather than on the profile), `cycle_events` (baseline), `user_devices` (push token), notification prefs.
...
- **`POST /me/devices` (P4a/T15):** push-token registration, called by the client **on every app start** and on every token refresh. **P4b obligation — this cadence is load-bearing, not cosmetic.** The detach's accepted cost (anyone holding a victim's token can unregister their device) is only bounded because the victim's app re-registers and takes the token back; "first launch + token refresh" is *not* self-healing, because a silenced user would stay silenced until FCM happens to rotate the token, potentially for months. P4b must register on every start. Body is `{ platform, pushToken }` — `platform` ∈ {`ios`, `android`}, `pushToken` 1–512 chars (the pre-existing column width, **not a P4a invention**), both required, trimmed before they are measured. It is an **UPSERT on the pre-existing unique `(UserId, PushToken)`** — found ⇒ `platform` + `last_seen_at` move, else insert — and answers **200 either way**: an upsert has no actionable created/updated distinction for a caller that re-registers on every refresh, and §C.9 exposes no `GET /me/devices/{id}` for a `Location` header. **The 200 body never carries the token** (`{ deviceId, platform, lastSeenAt, createdAt }`); an unknown platform, a blank token or an overlength one is the shared 400 and writes nothing; a missing `users` row is the shared 404, decided before validation. **Registering a token DETACHES it from every other account** — see the P4a row in §A. **No migration** (`user_devices` since `20260614150634`), and push-token-at-rest encryption stays out of scope (an open P9a precondition). **P9a obligations this endpoint creates, in full:** (1) **add `CREATE INDEX ON user_devices ("PushToken")`** — the detach looks up by token alone, but the only indexes are `PK(Id)` and `UNIQUE(UserId, PushToken)`, and Postgres 16 has no skip scan, so every registration is a **sequential scan** inside the write transaction (verified by `EXPLAIN` on the live DB); §G4 was spent before this was known, so P4a legitimately cannot fix it; (2) the dispatcher must **collapse, not merely tolerate**, more than one row per token — two concurrent registrations can still interleave, and preferring newest `last_seen_at` alone leaves the loser row until some account happens to re-register; (3) unregister-on-sign-out and delete-on-provider-`NotRegistered`; (4) note that a cross-tenant detach committing between another caller's read and update yields `DbUpdateConcurrencyException`, which `ConcurrencyRetry` does **not** retry (it matches only `23505`), so that narrow race surfaces as a 500 with no data loss.
