...
// Single copy of the DEK-custody logic shared by UserCryptoContext and JobCryptoContext.
/// <summary>
/// Base for envelope-crypto contexts. The DEK is unwrapped at most once per instance (guarded for
/// concurrent first-use), held only in this instance, and zeroed on disposal — so it never outlives
/// the owning scope and two instances never share key material. Subclasses supply only
/// <see cref="GetUserId"/>, resolved at the same point in the unwrap flow where each concrete
/// context resolves its user id today (request-scoped vs. job-scoped).
/// </summary>
public abstract class DekCryptoContextBase(
    LumenDbContext db,
    IKeyWrapper keyWrapper,
    IFieldCipher fieldCipher) : IAsyncDisposable
{
    private readonly SemaphoreSlim _gate = new(1, 1);
    private volatile byte[]? _dek;

    /// <summary>The id of the user whose DEK this context unwraps.</summary>
